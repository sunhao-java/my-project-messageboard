 ...
 *
 * <p>
 * How would we include the <em>hobbies</em> field?  Using the {@link com.wiscom.generic.flexjson.JSONSerializer#include}
 ...
 * those fields, and use the {@link com.wiscom.generic.flexjson.JSONSerializer#include} method.
 * </p>
 *
 * <p>
 ...
 * allow you to do this.  Flexjson ships with a simple HTML encoder {@link com.wiscom.generic.flexjson.HTMLEncoder}.
 ...
 * threads.  It is NOT safe to use combination of {@link com.wiscom.generic.flexjson.JSONSerializer#include(String[])}
 * {@link JSONSerializer#transform(Transformer, String[])}, or {@link com.wiscom.generic.flexjson.JSONSerializer#exclude(String[])}
 * from multiple threads at the same time.  It is also NOT safe to use
 * {@link com.wiscom.generic.flexjson.JSONSerializer#serialize(String, Object)} and include/exclude/transform from
...
public class JSONSerializer {
     ...
     * named rootName.  This is great to use in conjunction with other libraries
     * like EXTJS whose data models require them to be wrapped in a JSON object.
     * See {@link com.wiscom.generic.flexjson.JSONSerializer#deepSerialize(Object)} for more
    ...
    public JSONSerializer include( String[] fields ) {
        for( int i=0; i<fields.length; i++ ) {
            pathExpressions.add( new PathExpression( fields[i], true ) );
        }
        return this;
    }
    ...
    public JSONSerializer transform( Transformer transformer, String[] fields ) {
        for( int i=0; i<fields.length; i++ ) {
            if( fields[i].length() == 0 ) {
                ...
                transformations.put( new Path( fields[i].split("\\.") ), transformer );
            }
        }
        return this;
    }
    ...
    public List getIncludes() 
    {
        ...
        Iterator it = pathExpressions.iterator();
        while(it.hasNext())
        {
        	PathExpression expression = (PathExpression)it.next();
    ...
    private abstract class ObjectVisitor {
       ...
       // }
        ...
        private Object doTransform(Object value) {
            if( transformations.containsKey( path ) ) {
                value = ((Transformer)transformations.get( path )).transform( value );
            }
            return value;
        }
        ...
        protected PathExpression matches(PropertyDescriptor prop, List expressions) {
        	Iterator it = expressions.iterator();
        	while(it.hasNext())
        	{
            	PathExpression expr = (PathExpression)it.next();
    ...
    private class ShallowVisitor extends ObjectVisitor {
        ...
        protected boolean isIncluded( PropertyDescriptor prop ) {
            ...
            */
            ...
            return !(propType.isArray() || Collection.class.isAssignableFrom(propType) || Map.class.isAssignableFrom(propType));
        }
    }
...
//	    	
//	    	list.add(u);
//	    	//System.out.println(s.deepSerialize(u));
//    	}
//    	s.exclude(new String[]{"class"});
//    	System.out.println(s.deepSerialize(list));
    }
}
