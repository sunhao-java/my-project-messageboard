...
(function($){
	...
	$.editor.defaults = {
		...
		basePath: YAHOO.util.getContextPath(),
			...
			var f = {
				init: function(){
					...
					if(p.afterCreate && typeof(p.afterCreate) == 'string'){
						afterCreate = YAHOO.util.decode(p.afterCreate);
					}
					
					var afterChange = null;
					if(p.afterChange && typeof(p.afterChange) == 'string'){
						afterChange = YAHOO.util.decode(p.afterChange);
					}
					
					var uploadUrl = p.basePath + p.uploadUrl;
					
					var toolbar = YAHOO.util.toolbars[p.toolbar];
					if(!toolbar || toolbar.length == 0){
						toolbar = YAHOO.util.toolbars['default'];
					}
					...
					config.basePath = p.basePath + '/js/kindeditor/';
