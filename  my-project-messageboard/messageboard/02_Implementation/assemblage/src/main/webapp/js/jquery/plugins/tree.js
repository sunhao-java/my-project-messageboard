...
(function($)
{
    ...
    $.ligerDefaults.Tree = {
        ...
        disableFlag:'hasRole'         //灰色标记
    };

    $.tree = {};
    $.tree.addNode = function(treeParentNodeIndex, treeNodeName, reset)
    {
        if(reset){
            location.reload();
        }else {
    ...
    $.tree.editNode = function(treeNodeIndex, treeNodeName, reset)
    {
        if(reset){
            location.reload();
        }else {
            var manager = $(".l-tree").ligerGetTreeManager();
            //var node = manager.data[treeParentNodeIndex];
            //弹出编辑对话框
            var node = manager.getSelected();
            if (node){
                manager.update(node.target, { text: treeNodeName });
            }else{
                alert('请先选择节点');
            }
        }
    };
    $.tree.deleteNode = function(treeId, url, execFunc) {
        var manager = $("#" + treeId).ligerGetTreeManager();
        if (!manager)return;
        //先判断复选框是否有选中
        ...
        if (notes == null || notes.length == 0) {
            //如果复选框没选，则判断单击选中的行
            notes = manager.getSelected();
            if (notes != null){
                ids += notes.data.pkId + ",";
                names += notes.data.treeNodeName;
            }
        } else {
            for (var i = 0; i < notes.length; i++){
                ids += notes[i].data.pkId + ",";
                names += notes[i].data.treeNodeName + ",";
            }
        }

        if (ids == "") {
            top.$.ligerDialog.alert("请选择要删除的数据！", "提示", "warn");
            return;
        }
        top.$.ligerDialog.confirm("确定要删除“"+names+"”吗？", function(r) {
            if (r) {
                $.ajax({
                    type:'post',
                    url:url ? url : '',
        ...
        this.each(function()
        {
            ...
            var g = {
                ...
                hasChildren: function(treenodedata)
                {
                    ...
                    return treenodedata.children && treenodedata.children != "" ? true : false;
                },
                //获取父节点
                ...
                moveUp: function()
                {
                    var item =  $(".l-selected", g.tree).parent("li");
                ...
                getSelectedPrev: function()
                {
                    var node = {};
                    node.target = $(".l-selected", g.tree).parent("li").prev()[0];
                    if (node.target)
                    {
                        var treedataindex = parseInt($(node.target).attr("treedataindex"));
                        node.data = po.getDataNodeByTreeDataIndex(g.data, treedataindex);
                ...
                getSelectedNext: function()
                {
                    var node = {};
                    node.target = $(".l-selected", g.tree).parent("li").next()[0];
                    if (node.target)
                    {
                        var treedataindex = parseInt($(node.target).attr("treedataindex"));
                        node.data = po.getDataNodeByTreeDataIndex(g.data, treedataindex);
                ...
                loadData: function(node, url, param)
                {
                    ...
                    $.ajax({
                        ...
                        success: function(data)
                        {
                            if (!data) return;
                            g.loading.hide();
                            g.append(node, data.Rows);
                            if (p.onSuccess) p.onSuccess(data.Rows);
                        },
            ...
            var po = {
                ...
                getChildNodeClassName: function(data)
                {
                    //增加data保护 by zwzhao 20120726
                    var nodeclassname = 'l-tree-icon-' + p.childIcon
                    if (data && data.showInNavigation == 0 && data.enableStatus == 0) nodeclassname+='-dh';
                    else if (data && data.showInNavigation == 0) nodeclassname+='-h';
                    else if (data && data.enableStatus == 0) nodeclassname+='-d';
                    return nodeclassname;
                },
                getParentNodeClassName: function(data,isOpen)
                {
                ...
                getTreeHTMLByData: function(data, outlineLevel, isLast, isExpand)
                {
                    ...
                    for (var i = 0; i < data.length; i++)
                    {
                        ...
                        if (g.hasChildren(data[i]))
                        {
                            ...
                            p.parentIcon && !isExpandCurrent && treehtmlarr.push('<div class="l-box ' + po.getParentNodeClassName(data[i],false) + '"></div>');
                            p.parentIcon && isExpandCurrent && treehtmlarr.push('<div class="l-box ' + po.getParentNodeClassName(data[i],true) + '"></div>');
                        }
                        else
                        {
                            ...
                            p.childIcon && treehtmlarr.push('<div class="l-box ' + po.getChildNodeClassName(data[i]) + '"></div>');
                        }
                        if(data[i][p.disableFlag] == false){
                            treehtmlarr.push('<span class="gray">' + data[i][p.textFieldName] + '</span></div>');
                        }else{
                ...
                setTreeEven: function()
                {
                    p.onContextmenu && g.tree.bind("contextmenu", function(e)
                    {
                        ...
                        if (obj.tagName.toLowerCase() == "a" || obj.tagName.toLowerCase() == "span" || $(obj).hasClass("l-box"))
                            ...
                            treeitem = $(obj);
                        if (!treeitem) return;
                        if($("span",treeitem).hasClass("gray")){
                            return;
                        }
                    ...
                    g.tree.click(function(e)
                    {
                        ...
                        if (!$(obj).hasClass("l-checkbox") && !$(obj).hasClass("gray") && !$(obj).siblings().hasClass("gray") && !clickOnTreeItemBtn)
                        {
                            //再次点击仍然触发事件 update by junfang 20120312
                            /*if ($(">div:first", treeitem).hasClass("l-selected"))
                             ...
                             return false;
                             //产品提出再次点击不取消选中 update by junfang 20120228
                        ...
                        //chekcbox even
                        if ($(obj).hasClass("l-checkbox")&& !$(obj).siblings().hasClass("gray"))
                        {
                        ...
                        //状态：已经张开
                        else if (treeitembtn.hasClass("l-expandable-open") && clickOnTreeItemBtn)
                        {
                            if (p.onBeforeCollapse
                                && p.onBeforeCollapse({ data: treenodedata, target: treeitem[0] }) == false)
                                return false;

                            //产品提出，点击节点名不收缩树，点击+或者-才收缩树
                            //edit by sunhao(haosun@wisedu.com) 2012-02-29 11:24
                            if(!(obj.tagName.toLowerCase() == 'span')){
                                ...
                                $("> div ." + po.getParentNodeClassName(true), treeitem).removeClass(po.getParentNodeClassName(true))
                        ...
                        //状态：没有张开
                        else if (treeitembtn.hasClass("l-expandable-close") && clickOnTreeItemBtn)
                        {
                            ...
                            if(!(obj.tagName.toLowerCase() == 'span')){
                                ...
                                $("> div ." + po.getParentNodeClassName(), treeitem).removeClass(po.getParentNodeClassName())
                                    .addClass(po.getParentNodeClassName(true));
                            }
