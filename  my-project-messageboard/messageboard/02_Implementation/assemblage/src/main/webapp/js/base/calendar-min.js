...
var topY=0,leftX=0;  //自定义定位偏移量 2007-02-11 由 添加   
...
{   
    ...
    topY  =(y == null) ? topY : y;//自定义定位偏移量 2007-02-11 由 添加    
 ...
 * @param p the pattern of your date  
 2006-06-25 由  修改为根据用户指定的 style 来确定；  
 */  
...
Calendar.prototype.draw = function() {   
  ...
  var mvAry = [];   
  //mvAry[mvAry.length]  = '  <form name="calendarForm" style="margin: 0px;">'; //因 <form> 不能嵌套， 2006-12-01 由改用 Div   
...
//2009-03-03 添加的代码，放置时间的行   
  ...
  { calendar.ReturnDate(""); /*calendar.dateControl.value = "";calendar.hide();*///2007-09-14 由注释   
  }   
...
Calendar.prototype.bindYear = function() {   
  var cy = this.calendarYear;//2006-12-01 由修改   
  cy.length = 0;   
...
Calendar.prototype.bindMonth = function() {   
  var cm = this.calendarMonth;//2006-12-01 由修改   
  cm.length = 0;   
  for (var i = 0; i < 12; i++){   
...
Calendar.prototype.update = function (e){   
  this.year  = e.calendarYear.options[e.calendarYear.selectedIndex].value;//2006-12-01 由修改   
...
Calendar.prototype.show = function (dateObj, popControl) {   
  ...
  var now =  new Date();   
  this.date = (dateObj.value.length > 0) ? new Date(dateObj.value.toDate(this.dateFormatStyle)) : now.format(this.dateFormatStyle).toDate(this.dateFormatStyle) ;//2008-01-29 添加 → 若为空则根据dateFormatStyle初始化日期   
  ...
  this.panel.style.left = (xy.x + leftX)+ "px"; //由 2007-02-11 修改 → 加入自定义偏移量   
  this.panel.style.top = (xy.y + topY + dateObj.offsetHeight) + "px";   
     
  //由 2006-06-25 修改 → 把 visibility 变为 display，并添加失去焦点的事件  //this.setDisplayStyle("select", "hidden");   
